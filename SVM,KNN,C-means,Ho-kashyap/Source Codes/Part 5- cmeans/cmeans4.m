function [train_dataset] = cmeans4 (clusters)
  clear all;
  clusters=4;
  ...
  while (abs(err) > 0.000001)
    ...
    % Euclidean distance
      for j=1:clusters
        ...
        dist(j) = sum(abs(v)); %manhattan distance 7,4,3
        %dist(j) = norm(v,2); % Euclidean distance 3,4,7
    ...
    err = sum (abs(prior_mean(1,:) - mean(cluster1) + prior_mean(2,:) - mean(cluster2) + prior_mean(3,:) - mean(cluster3) + prior_mean(4,:) - mean(cluster4) ))
    prior_mean= [mean(cluster1);mean(cluster2);mean(cluster3);mean(cluster4)]
    i1
    i2
    i3
    i4
